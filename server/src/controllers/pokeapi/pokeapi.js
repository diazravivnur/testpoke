...
exports.getListPoke = async (req, res) => {
  try {
    ...
    // const dataArray = getData.results;
    // const createPoke = await poke.bulkCreate(dataArray, {
    //   fields: ["name", "url"],
    ...
    // const data = await getData.results.name;

    // for (let index = 0; index < pokeName.length; index++) {
    //   const element = pokeName[index];
    //   console.log(element);
    // }
    // ADD TO MODEL
...
exports.catchPokemon = async (req, res) => {
  ...
  try {
    ...
    function isEven(theRandomNumber) {
    ...
    console.log("iseven", isEven(theRandomNumber));
    if (isEven(theRandomNumber) === "ganjil") {
      return res.status(404).send({
        status: "failed",
        message: `You Failed Catch Pokemon because your random number is ${theRandomNumber} it's odd number`
      });
    }
